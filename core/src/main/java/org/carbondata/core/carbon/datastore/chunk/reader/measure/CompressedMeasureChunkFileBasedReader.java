...
public class CompressedMeasureChunkFileBasedReader extends AbstractMeasureChunkReader {
	 ...
	 *            measure chunk metadata
	 * @param compression
	 *            model metadata which was to used to compress and uncompress
	 *            the measure value
	 * @param filePath
	 ...
	 * @param fileReader
	 * 			file reader to read the blocks 
	 * @param blockIndexes
	 * 			blocks to be read 
	...
	public MeasureColumnDataChunk readMeasureChunk(FileHolder fileReader,
		...
		copy.setValue(fileReader.readByteArray(filePath, measureColumnChunk
				.get(blockIndex).getDataPageOffset(),
				measureColumnChunk.get(blockIndex).getDataPageLength()));
		// get the data holder after uncompressing 
		CarbonReadDataHolder measureDataHolder=copy.uncompress(
				compressionModel.getChangedDataType()[blockIndex]).getValues(
