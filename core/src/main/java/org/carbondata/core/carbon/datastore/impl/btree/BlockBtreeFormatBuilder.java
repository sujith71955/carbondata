...
public class BlockBtreeFormatBuilder extends AbstractBtreeFormatBuilder {
	...
	private static final LogService LOGGER = LogServiceFactory
			.getLogService(BlockBtreeFormatBuilder.class.getName());
	 ...
	 */
	@Override
	public void build(BlocksBuilderInfos segmentBuilderInfos) {
		long totalNumberOfTuple = 0;
		int groupCounter;
		int nInternal = 0;
		BTreeNode curNode = null;
		BTreeNode prevNode = null;
		List<BTreeNode[]> nodeGroups = new ArrayList<BTreeNode[]>(
				CarbonCommonConstants.CONSTANT_SIZE_TEN);
		BTreeNode[] currentGroup = null;
		List<List<IndexKey>> interNSKeyList = new ArrayList<List<IndexKey>>(
				CarbonCommonConstants.CONSTANT_SIZE_TEN);
		List<IndexKey> leafNSKeyList = null;
		long nodeNumber=0;
		for (int index = 0; index < segmentBuilderInfos.getDataFileMetadataList().get(0)
				.getLeafNodeList().size(); index++) {
			// creating a leaf node
			curNode = new BlockBTreeLeafNode(segmentBuilderInfos,index,nodeNumber++);
			totalNumberOfTuple += segmentBuilderInfos.getDataFileMetadataList().get(0).getLeafNodeList()
			...
			if (groupCounter == 0) {
				...
				leafNSKeyList = new ArrayList<IndexKey>(
						CarbonCommonConstants.CONSTANT_SIZE_TEN);
			...
			if (null != leafNSKeyList) {
				leafNSKeyList
						.add(convertStartKeyToNodeEntry(segmentBuilderInfos
								.getDataFileMetadataList().get(0).getLeafNodeList()
								.get(index).getLeafNodeIndex().getBtreeIndex()
								.getStartKey()));
			}
