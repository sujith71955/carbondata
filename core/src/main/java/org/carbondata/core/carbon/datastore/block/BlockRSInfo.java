...
public class BlockRSInfo {
	...
	/**
	 * below method will fill detail from block metadata 
	 * @param blockMetadata
	 */
	private void fillDetailFromBlockMetadata(BlockMetadata blockMetadata) {
		...
		for (int i = 0; i < columnInTable.size(); i++) {
			...
			if (columnSchema.isDimensionColumn()) {
				CarbonDimension carbonDimension = new CarbonDimension(
						columnSchema, dimensonOrdinal++);
				// not adding the cardinality of the non dictionary column as it was not the part of mdkey
				if (!carbonDimension.getEncodingList().contains(
						Encoding.DICTIONARY)) {
		...
		dimColumnCardinality = new int[cardinalityIndexForDimensionColumn
				.size()];
		int index = 0;
		...
		for (Integer cardinalityArrayIndex : cardinalityIndexForDimensionColumn) {
			dimColumnCardinality[index++] = blockMetadata.getSegmentInfo()
					.getColumnCardinality()[cardinalityArrayIndex];
		}
		dimensions = CarbonUtil.arrangeDimension(dimensions);
		this.normalDimensionKeyGenerator = KeyGeneratorFactory
				.getKeyGenerator(dimColumnCardinality);
	}
