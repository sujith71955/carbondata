...
public class CompressedDimensionChunkFileBasedReader extends AbstractChunkReader {
	 ...
	 * 
	 * @param dimensionColumnChunk
	 *            dimension chunk metadata
	 ...
	 * @param fileReader
	 * 			file reader to read the blocks from file 
	 * @param blockIndexes	
	 * 			blocks to be read 
	...
	public DimensionColumnDataChunk readDimensionChunk(FileHolder fileReader,
		...
		dataPage = COMPRESSOR.unCompress(fileReader.readByteArray(filePath,
		...
		if (dimensionColumnChunk.get(blockIndex).getEncodingList()
			...
			invertedIndexes = CarbonUtil.getUnCompressColumnIndex(
					...
					fileReader.readByteArray(filePath, dimensionColumnChunk
							.get(blockIndex).getRowIdPageOffset(),
							dimensionColumnChunk.get(blockIndex)
									.getRowIdPageLength()), numberComressor);
