...
public class DriverBtreeFormatBuilder extends AbstractBtreeFormatBuilder {
	...
	private static final LogService LOGGER = LogServiceFactory
			.getLogService(DriverBtreeFormatBuilder.class.getName());
	...
	public void build(BlocksBuilderInfos segmentBuilderInfos) {
		...
		long nodeNumber=0;
		for (int metadataIndex = 0; metadataIndex < segmentBuilderInfos
				.getDataFileMetadataList().size(); metadataIndex++) {
			// creating a leaf node
			curNode = new DriverBtreeLeafNode(segmentBuilderInfos,
					metadataIndex,nodeNumber++);
			nLeaf++;
			...
			if (null != leafNSKeyList) {
				leafNSKeyList
						.add(convertStartKeyToNodeEntry(segmentBuilderInfos
								...
								.getLeafNodeIndex().getBtreeIndex()
								.getStartKey()));
			}
