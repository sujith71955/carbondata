...
class NO_DICTIONARY_COL_TestCase extends QueryTest with BeforeAndAfterAll {
  ...
  override def beforeAll {
    ...
    sql("LOAD DATA fact from 'D:/carbondata/integration/spark/src/test/resources/data.csv' INTO CUBE NO_DICTIONARY_CARBON_3 PARTITIONDATA(DELIMITER ',', QUOTECHAR '\"')");
  ...
  test("Detail Query with NO_DICTIONARY_COLUMN Compare With HIVE RESULT") {
    ...
    checkAnswer(
      sql("select empno from NO_DICTIONARY_CARBON_3"),
   ...
   test("Filter Query with NO_DICTIONARY_COLUMN Compare With HIVE RESULT") {
     ...
     checkAnswer(
      sql("select empno from NO_DICTIONARY_HIVE_3 where empno=15"),
      sql("select empno from NO_DICTIONARY_CARBON_3 where empno=15"))
   }
   
    test("Filter Query with NO_DICTIONARY_COLUMN and DICTIONARY_COLUMN Compare With HIVE RESULT") {
    
     checkAnswer(
      sql("select empno from NO_DICTIONARY_HIVE_3 where empno=15 and deptno=12"),
      sql("select empno from NO_DICTIONARY_CARBON_3 where empno=15 and deptno=12"))
   }
    
    test("Distinct Query with NO_DICTIONARY_COLUMN  Compare With HIVE RESULT") {
    
     checkAnswer(
      sql("select count(distinct empno) from NO_DICTIONARY_HIVE_3"),
      sql("select count(distinct empno) from NO_DICTIONARY_CARBON_3"))
   }
